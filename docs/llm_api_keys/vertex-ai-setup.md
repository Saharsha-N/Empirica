...
Empirica agents built with LangGraph can be run using a Gemini API key (see above). However, agents built using [AG2](https://ag2.ai/) require a different setup to access Gemini models via the [Vertex AI](https://cloud.google.com/vertex-ai?hl=en) API.
...
2. **Download** the file to the machine where Empirica will run.
...
This environment variable must be set whenever Empirica requires Vertex AI access.
...
- Click "Select a project" (top left) and choose an existing project or click "New project" (top right) to create a new one (e.g., "empirica").
...
- Name the account (e.g., "empirica"). The description is optional.
- Click "Create and Continue".
- In "Select a Role", enter "Vertex AI User" and select it. Click "Continue".
- Skip "Principals with access". Click "Done".
- In the list view, find your account (e.g., `empirica@empirica-1234.iam.gserviceaccount.com`).
