...
Empirica can fail for many different reason. Here we describe some standard situations and how to deal with them.

## Empirica failed when performing analysis with `get_results`

Empirica makes use of [cmbagent](https://github.com/CMBAgents/cmbagent) as a backend for the analysis. Check carefully the logs and see what was the last stepts it was doing.
...
- **Failed because of timed out**. Some calculations may take longer than expected (or time allocated). In this case, one can rerun the module restarting it from the step it failed. For this, rerun empirica with

```python
from empirica import Empirica

# Follow the research plan, write and execute code, make plots, and summarize the results
emp.get_results(engineer_model='gemini-2.5-pro',
...
- **Failed because could not fix bug**. Sometimes, Empirica will write a piece of code that will fail. It will then try to fix the problem (sometimes just a bug). However, in some cases it will not manage to do it. Sometimes this happens in the early steps when just reading the data and doing some prepreocessing. In this case, we recommend rerunning the module from the beginning, i.e.
...
- **It did not generate any plot**. Sometimes, even when performing complex numeric computations, Empirica may not generate any plot at all. Editing the input data description including the requirement of generating plots usually ensures that these are created.

## Empirica failed when writing the paper

This module will provide a status indicator ([SUCCESS] or [ERROR]) after each section it writes. Sometimes it may fail at different parts. For instance, it may have written properly the abstract, introduction, results and conclusions, but it may have failed in the methods section. Empirica will try to fix the LaTeX problems but in some cases it wont be able to do it. In this situation, we recommend identifying the problematic part (methods in this case) and go to `paper/temp/methods.tex` and erase that file. Next, rerun the module:

```python
from empirica import Empirica, Journal

# Write a paper with [APS (Physical Review Journals)](https://journals.aps.org/) style
emp.get_paper(journal=Journal.AAS, llm='gemini-2.5-flash', add_citations=False)
```
...
We recommend checking all versions, and not just the last one, as in some cases some plots may be missing. If figures are in version1, but missing in version2, remove all the results files `paper/temp/Results*` and rerun Empirica.

Sometimes Empirica may have problems inserting figures in the text. In this case, as above, we recommend checking the `paper/temp` folder and removing the problematic files.
...
Here are some tips to have in mind when running Empirica to have better results:
