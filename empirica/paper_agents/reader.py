...
def preprocess_node(state: GraphState, config: RunnableConfig):
    ...
    for f in journal_files:
        source_file = Path(LaTeX_DIR) / f
        # Copy to Paper_folder
        dest_file = Path(state['files']['Paper_folder']) / f
        if not dest_file.exists() and source_file.exists():
            try:
                shutil.copy2(str(source_file), str(dest_file))
            except (OSError, shutil.Error) as e:
                logger.warning(f"Failed to copy {f} to Paper_folder: {e}")
        # Copy to Temp
        dest_file = Path(state['files']['Temp']) / f
        if not dest_file.exists() and source_file.exists():
            try:
                shutil.copy2(str(source_file), str(dest_file))
            except (OSError, shutil.Error) as e:
                logger.warning(f"Failed to copy {f} to Temp: {e}")
