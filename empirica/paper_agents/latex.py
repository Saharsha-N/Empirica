...
def extract_latex_errors(state):
    ...
    with open(state['files']['LaTeX_log'], 'r', encoding='utf-8') as f:
        lines = f.readlines()
    ...
    with open(state['files']['LaTeX_err'], 'w', encoding='utf-8') as f:
        if errors:
            f.write("LaTeX Compilation Errors Found:\n\n")
            for error in errors:
                f.write(error + "\n\n")
        else:
            f.write("[SUCCESS] No LaTeX errors found.\n")
...
def clean_files(doc_name, doc_folder):
    ...
    for suffix in ['aux', 'log', 'pdf', 'out']:
        file_to_remove = Path(doc_folder) / f'{doc_stem}.{suffix}'
...
def compile_tex_document(state: dict, doc_name: str, doc_folder: str) -> None:
    ...
    def run_xelatex(pass_num=None):
        ...
        if result.returncode != 0:
            print("[ERROR]", end="", flush=True)  # Inline progress indicator
            logger.error("XeLaTeX compilation failed")
    ...
    def log_output(result):
        with open(state['files']['LaTeX_log'], 'a', encoding='utf-8') as f:
            f.write("---- STDOUT ----\n")
    ...
    print("[SUCCESS]", end="", flush=True)  # Inline progress indicator
...
def compile_latex(state: GraphState, paper_name: str) -> None:
    ...
    try:
        run_xelatex()
        print("[SUCCESS]", end="", flush=True)  # Inline progress indicator
        logger.info("LaTeX compilation pass 1 succeeded")
    except subprocess.CalledProcessError as e:
        log_output("Pass 1", e, is_error=True)
        print("[ERROR]", end="", flush=True)  # Inline progress indicator
        logger.error("LaTeX compilation pass 1 failed", exc_info=True)
    ...
    for i in range(further_iterations):        
        try:
            run_xelatex()
            print("[SUCCESS]", end="", flush=True)  # Inline progress indicator
            logger.info(f"LaTeX compilation final pass {i+1} succeeded")
        except subprocess.CalledProcessError as e:
            log_output(f"Final Pass {i+1}", e, is_error=True)
            print("[ERROR]", end="", flush=True)  # Inline progress indicator
            logger.error(f"LaTeX compilation final pass {i+1} failed", exc_info=True)
...
def process_bib_file(input_file, output_file):
    ...
    logger.info(f"Sanitized BibTeX saved to: {output_file}")
...
def fix_latex(state, f_temp):
    ...
    for i in range(3):
        ...
        if compile_tex_document(state, f_name, state['files']['Temp']):
            try:
                f_temp_path = Path(f_temp)
                f_orig_path = Path(f"{f_stem}_orig{suffix}")
                f_name_path = Path(f_name)
                # Move original to backup
                if f_temp_path.exists():
                    shutil.move(str(f_temp_path), str(f_orig_path))
                # Move new version to original location
                if f_name_path.exists():
                    shutil.move(str(f_name_path), str(f_temp_path))
