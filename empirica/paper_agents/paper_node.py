...
def _import_fitz():
    """Lazy import of PyMuPDF (fitz) with helpful error message."""
    try:
        import fitz  # PyMuPDF
        return fitz
    except ImportError:
        raise ImportError(
            "PyMuPDF (pymupdf) is required for this functionality but is not installed. "
    ...
    if f_temp.exists():
        ...
        if state['paper']['cmbagent_keywords']:
            ...
            try:
                ...
                raise ImportError(
                    "cmbagent is required for cmbagent_keywords functionality but is not installed. "
...
async def citations_node(state: GraphState, config: RunnableConfig):
    ...
    print("[SUCCESS] Citations added to all sections.")
